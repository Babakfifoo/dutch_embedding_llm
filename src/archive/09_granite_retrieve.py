...
logging.basicConfig(
    ...
    encoding='utf-8',
    level=logging.INFO,
    format='%(asctime)s:%(levelname)s:granite:2b:%(message)s')
...
for i, (IMRO, plan_string) in tqdm(enumerate(data.items())):
    if plan_string == []:
        continue
    granite_answer: ChatResponse = chat(
        model='granite3.1-dense:2b',
        messages=[{
            'role': 'user',
            'content': granite_prompt_template.format(plan_string=plan_string[0]["text"]),
            }]
        )
    granite_answers[IMRO] = granite_answer['message']["content"]
    logging.info(msg=f"{IMRO} --> mistral Answered")
    if (i+1) % 100 == 0:
        with open(f"../data/granite.json", "w") as file:
            file.write(json.dumps(granite_answers, indent=4))
