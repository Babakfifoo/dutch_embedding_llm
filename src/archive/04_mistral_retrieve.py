...
logging.basicConfig(
    ...
    encoding='utf-8',
    level=logging.INFO,
    format='%(asctime)s:%(levelname)s:Mistral:%(message)s')
...
for i, (IMRO, plan_string) in tqdm(enumerate(data.items())):
    ...
    if plan_string == []:
        continue
    mistral_answer: ChatResponse = chat(
        model='mistral',
        messages=[{
            'role': 'user',
            'content': mistral_prompt_template.format(plan_string=plan_string[0]["text"]),
            }]
        )
    mistral_answers[IMRO] = mistral_answer['message']["content"]
    logging.info(msg=f"{IMRO} --> mistral Answered")
    if (i+1) % 100 == 0:
        with open(f"../data/02_mistral/{i+1}.json", "w") as file:
            file.write(json.dumps(mistral_answers, indent=4))
            mistral_answers = {}
